...
{ 
      ...
      for(std::list<Individu>::iterator cur = lIndividu.begin(); cur != lIndividu.end(); cur++)
	{
	  if((*cur).gen == noGen)
	    { 
		 ...
		 {participe[k]=0;}
		  ...
		  for(int k=((int)lIndividu.size())-1;k>=0;k--)
			...
			{ 
			  ...
			  if(ind.origine==2||ind.origine==3)
			    {
			      if(tabInd[ind.parent1].fitness < tabInd[ind.parent2].fitness || ind.parent2==0)
		  ...
		  std::string fichier  = dest_file_name;
		  std::ostringstream flport;
		  flport << (*cur).id;
		  std::string exte = flport.str();
		  fichier.append(exte);
		  fichier.append(".csv");
		  std::ofstream dest_file(fichier.c_str(), std::ios::out | std::ios::trunc);
		  if(!dest_file)
		    throw std::string("can't open file \"" + dest_file_name + "\"");
        
		  dest_file << "GEN;ID;FITNESS;GAINFITNESS;SURVIVAL;ORIGINE" << std::endl;
		  ecrireInfo(tabInd,participe,(int)lIndividu.size(),fichier);
		  dest_file.close();
		  participe[0]=1;
      ...
      /* ------------------------------------------------- */
		  ...
		  std::string fichier2  = gen_file_name;
		  std::ostringstream flport2;
		  flport2 << (*cur).id;
		  std::string exte2 = flport2.str();
		  fichier2.append(exte2);
		  fichier2.append(".txt");

		  std::ofstream gen_file(fichier2.c_str(), std::ios::out | std::ios::trunc);
		  if(!gen_file)
		  ...
		  while(!src_file2.eof())
		      ...
		      else if(type == "Cross" || type == "Both")
			{
			  ...
			  if(participe[id_son] > 0) 
			    {
			      if( (tabInd[id_parent1].fitness < tabInd[id_parent2].fitness) || id_parent2 == 0) gen_file << type <<" " << semicolon<<" " << sNoGen<<" " << noGen<<" " << tmp1<<" " << sId_parent1<<" " << id_parent1<<" " << tmp2<<" " << sId_parent2<<" " << 0 <<" " << tmp3<<" " << sId_son<<" " << id_son << std::endl;
			      else gen_file << type <<" " << semicolon<<" " << sNoGen<<" " << noGen<<" " << tmp1<<" " << sId_parent1<<" " << 0 <<" " << tmp2<<" " << sId_parent2<<" " << id_parent2 <<" " << tmp3<<" " << sId_son<<" " << id_son << std::endl;
			    }
			}
			...
			{
			  src_file2.ignore(NB_CHAR_MAX_PER_LINE,'\n');
			}
      ...
      catch(const std::string& error)
	{
	  ...
	  std::cout << "Use: lignee <src file> <stats file> <gen file>" << std::endl;
	  return 1;
	} 
      return 0;
}
